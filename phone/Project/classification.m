classdef classification
    ...
    methods(Static)
        function features = extractFeatures(acc, gyro, orientation, time)
            % extractFeatures - Extracts statistical and temporal features.
            features.statistical = classification.getStatisticalFeatures(acc, gyro, orientation);
            features.temporal = classification.getTemporalFeatures(acc, gyro, orientation, time);
        end
        
        function features = getStatisticalFeatures(accData, gyroData, orientData)
            ...
            features = struct();
            sensors = {accData, gyroData, orientData};
            sensorNames = {'acc', 'gyro', 'orient'};
            axisNames = {'x', 'y', 'z'};
        ...
        function features = getTemporalFeatures(accData, gyroData, orientData, time)
