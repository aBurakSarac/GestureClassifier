...
function main()
    ...
    try
        ...
        prepareGestureData(cfg.GestureFolder, targetGestures, learnerDir);
        displayInstructions();
    catch ME
        rethrow(ME);
    end
...
function prepareGestureData(dataFolder, targetGestures, outputDir)
    ...
    saveProcessedData(featureTable, outputDir);
end
...
function [vector, names] = convertFeaturesToVector(features)
    ...
    [totalElements, totalNames] = countFeatureElements(features);
    vector = zeros(1, totalElements);
    names = cell(1, totalNames);
    [vector, names, ~, ~] = fillFeatureArrays(features, vector, names, 1, 1);
end
...
function [numElements, numNames] = countFeatureElements(features)
    ...
    for i = 1:length(fields)
        ...
        if isstruct(field)
            [subElements, subNames] = countFeatureElements(field);
...
function [vector, names, nextVectorIdx, nextNameIdx] = fillFeatureArrays(features, vector, names, vectorIdx, nameIdx)
    ...
    for i = 1:length(fields)
        field = features.(fields{i});
        if isstruct(field)
            [vector, names, vectorIdx, nameIdx] = fillFeatureArrays(field, vector, names, vectorIdx, nameIdx);
        else
            fieldValues = field(:)';
...
function saveProcessedData(featureTable, outputDir)
    ...
    createDocumentation(outputDir, size(featureTable, 2) - 1);
end
